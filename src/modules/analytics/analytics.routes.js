...
const filterParams = {
  departmentId: { type: 'string', description: 'Filter results to a specific department ID' },
  from: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Start date YYYY-MM-DD (overrides range)' },
  to: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'End date YYYY-MM-DD (overrides range)' },
};
  ...
  fastify.get('/analytics/attendance', {
    schema: {
      ...
      querystring: {
        type: 'object',
        properties: {
          range: { type: 'string', enum: ['7d', '30d', '90d'], description: 'Preset date range' },
          ...filterParams,
        },
      },
      response: {
  ...
  fastify.get('/analytics/workforce-trend', {
    schema: {
      ...
      querystring: {
        type: 'object',
        properties: {
          range: { type: 'string', enum: ['6m', '12m', '2y'] },
          ...filterParams,
        },
      },
