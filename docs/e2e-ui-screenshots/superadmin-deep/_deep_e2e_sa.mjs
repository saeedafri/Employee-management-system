/**
 * SUPER_ADMIN full-depth UI E2E → docs/e2e-ui-screenshots/superadmin-deep/
 * Based on proven v3 runner + deeper nesting (modals/tabs/exports/detail).
 * Exclusive lock prevents concurrent harness races.
 */
...
const LOCK_FILE = path.join(OUT, '_OWNER.lock');
const PID_FILE = path.join(OUT, '_runner.pid');

// Exclusive single-runner guard
{
  ...
  if (fs.existsSync(PID_FILE)) {
    ...
    try {
      process.kill(old, 0);
      console.error(`ABORT: another SA deep runner pid=${old} holds ${PID_FILE}`);
      process.exit(2);
    } catch {
      /* stale */
    }
  ...
  fs.writeFileSync(
    LOCK_FILE,
    `owner=sa-deep-v3 pid=${process.pid} ts=${new Date().toISOString()}\n`,
  );
  const clear = () => {
    try {
...
const SETTINGS_SUB = [
  ...
  'billing-plan', 'billing-invoices', 'roles-permissions',
];
...
const REPORT_ROUTES = [
  '/reports?report=workforce/headcount',
  '/reports?report=workforce/turnover',
  '/reports?report=workforce/demographics',
  '/reports?report=attendance/summary',
  '/reports?report=leave/utilization',
  '/reports?report=payroll/ctc-analysis',
];
...
async function shot(page, name) {
  ...
  try {
    ...
    if (shotIdx % 10 === 0) console.log(`  📸 ${file}`);
    return file;
  } catch {
    try {
...
async function exploreDialogNest(page, state, menu, depth) {
  ...
  const tc = Math.min(await tabs.count().catch(() => 0), 6);
  for (let i = 0; i < tc; i++) {
  ...
  const btns = page.locator(
    '[role="dialog"] button:visible, [role="alertdialog"] button:visible',
  );
  const bc = Math.min(await btns.count().catch(() => 0), 8);
  for (let i = 0; i < bc; i++) {
    const b = btns.nth(i);
    ...
    if (!PRIORITY_RE.test(lab) && !/next|back|add|upload|browse|select/i.test(lab)) continue;
...
async function clickActions(page, menu, state, { home, max = 18 } = {}) {
  let clicked = 0;
  let detailNavs = 0;
  ...
  for (const m of metas) {
    ...
    const mic = Math.min(await menuItems.count().catch(() => 0), 8);
    if (mic > 0 && (/export|download|pdf|excel|csv|xlsx|columns|actions for|more/i.test(m.lab) || /^actions for /i.test(m.lab))) {
      for (let si = 0; si < mic; si++) {
        ...
        if (/pdf|excel|csv|xlsx|export|download/i.test(slab + m.lab)) counters.exportsTried += 1;
        await item.click({ force: true, timeout: 1500 }).catch(() => {});
        counters.buttons += 1;
        await settle(page, 900);
        const sss = await shot(page, `${menu}-menu-${slab}`);
        const newDl = downloads.slice(d2);
        const fails = state.failedRequests.slice(b2);
        const success = newDl.some((d) => d.ok);
        logF(menu, `menu:${slab}`, success || fails.length === 0 ? 'PASS' : 'FAIL', {
          screenshot: sss,
          downloads: newDl,
        });
        if (/export|download|pdf|excel|csv|xlsx/i.test(slab + m.lab)) {
        ...
        if (await nestedDlg.isVisible({ timeout: 250 }).catch(() => false)) {
          ...
          !String(menu).includes('>detail') &&
          /view profile|edit|open/i.test(slab) &&
          page.url() !== beforeUrl &&
          detailNavs < 1
        ) {
          detailNavs += 1;
          maxDepthReached = Math.max(maxDepthReached, 1);
          await clickActions(page, `${menu}>detail`, state, { home, max: 10 });
          if (home) await gotoMenu(page, home);
        }
        await hardDismiss(page);
      }
      void beforeDl;
    ...
    if (await dlg.isVisible({ timeout: 250 }).catch(() => false)) {
      if (/add|create|new|edit|invite|request|regulariz/i.test(m.lab) && !isApprove) {
    ...
    // Detail page dive (one max; never from a detail layer)
...
async function exploreDashboard(page, state) {
  ...
  await clickActions(page, 'Dashboard', state, { home: '/dashboard', max: 16 });
...
async function exploreMenu(page, menu, state) {
  ...
  if (!doneMenus.has(menu.label)) {
    ...
    await clickActions(page, menu.label, state, { home: menu.href, max: 20 });
  }

  if (menu.label === 'Reports') {
    for (const href of REPORT_ROUTES) {
      console.log(`  report ${href}`);
      await gotoMenu(page, href);
      const b = state.failedRequests.length;
      const se = await shot(page, `reports-${slug(href)}`);
      harvest(state, `Reports${href}`, se, b, 'open');
      await clickActions(page, `Reports${href}`, state, { home: href, max: 10 });
    }
  }
  ...
  if (menu.label === 'Settings') {
    for (const sub of SETTINGS_SUB) {
      ...
      const max = sub.includes('country-bank') ? 6 : 12;
  ...
  if (!doneMenus.has(menu.label)) {
    for (const extra of EXTRA[menu.label] || []) {
      ...
      await clickActions(page, `${menu.label}${extra}`, state, { home: extra, max: 12 });
    }
...
function writeOutputs() {
  ...
  const lines = [];
  lines.push('# SUPER_ADMIN Full-Depth Nested UI E2E Findings');
  lines.push('');
  lines.push(`- Generated: ${new Date().toISOString()}`);
  lines.push(`- Role: SUPER_ADMIN (${EMAIL})`);
  lines.push(`- UI: ${UI}`);
  lines.push(`- API: ${API}`);
  lines.push(`- Tenant: ${TENANT}`);
  lines.push('- MSW: OFF');
  lines.push('- Tool: Playwright Chromium (full-depth nested → superadmin-deep)');
  lines.push('- Screenshots: `docs/e2e-ui-screenshots/superadmin-deep/`');
  lines.push('- **No Render deploy. No git commit.**');
  lines.push(`- Menus completed: ${[...doneMenus].join(', ') || '(in progress)'}`);
  lines.push('');
  ...
  lines.push(`| Layers explored | ${counters.layers} |`);
  lines.push(`| Max nest depth | ${maxDepthReached} |`);
  lines.push(`| Exports tried | ${counters.exportsTried} |`);
  lines.push(`| Screenshots (counter) | ${counters.screenshots} |`);
  ...
  const crit = issues.filter(
    (i) =>
      /NO_EMPLOYEE_RECORD|critical|cross-user|data exposure|same key|access restricted|export produced no|400|500/i.test(
        `${i.title} ${i.why}`,
      ),
  );
  const top = (crit.length ? crit : issues).slice(0, 12);
  if (!top.length) lines.push('_None recorded yet._');
  top.forEach((iss, i) => {
    lines.push(
      `${i + 1}. **${iss.id}** [${iss.classification}] ${iss.title} — ${(iss.why || '').slice(0, 180)}`,
    );
  });
  lines.push('');
  ...
  fs.writeFileSync(
    path.join(OUT, '_run-raw.json'),
    JSON.stringify(
      { counters, maxDepthReached, findings, issues, downloads, doneMenus: [...doneMenus] },
      null,
      2,
    ),
  );

  // Only rewrite SUPER_ADMIN contracts when we have meaningful depth
  if (pngCount < 80 || doneMenus.size < 8) {
    console.log(`skip contract rewrite (png=${pngCount} menus=${doneMenus.size})`);
    return;
  }
  ...
  for (const [file, side] of [
    ...
    const sec = [
      ...
      '## SUPER_ADMIN',
      '',
      `> Updated ${new Date().toISOString()} — full-depth UI E2E vs ${UI} / ${API}`,
      `> Evidence: \`docs/e2e-ui-screenshots/superadmin-deep/\` (${pngCount} PNGs + FINDINGS.md)`,
      `> Depth: menus **${doneMenus.size}** · buttons/actions **${counters.buttons}** · maxDepth **${maxDepthReached}** · shots **${counters.screenshots}**`,
    ...
    for (const iss of filtered) {
      ...
      sec.push(
        `- Screenshot: ${iss.screenshot ? `docs/e2e-ui-screenshots/superadmin-deep/${iss.screenshot}` : 'n/a'}`,
      );
...
async function main() {
  ...
  if (fresh) {
    for (const f of fs.readdirSync(OUT)) {
      if (
        /^\d{2,3}-.*\.png$/.test(f) ||
        ['FINDINGS.md', '_run-raw.json', '_progress.json', 'results.json'].includes(f)
      ) {
        try {
