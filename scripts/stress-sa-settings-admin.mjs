/**
 * STRESS + DEEP E2E SHORT — SUPER_ADMIN settings/admin shard
 * ONLY: Permissions, Settings (all subroutes), Recruitment, Performance, Assets, Announcements
 * Stress: rapid-open 5 settings routes; capture console/network failures.
...
const SHOT =
  ...
  '/Users/mohdsaeedafri/All-Code-Base/EMS/docs/e2e-ui-screenshots/stress/sa-settings-admin';
const DOCS = '/Users/mohdsaeedafri/All-Code-Base/EMS/docs';
const EMAIL = 'superadmin@acme.test';
const PASS = 'Password123!';
const TENANT = 'acme-corp-001';
const ROLE = 'SUPER_ADMIN';
const SECTION = 'SA-SETTINGS-ADMIN';

const MAX_DEPTH = 2;
const MAX_CONTROLS_PER_LAYER = 14;
const MAX_CANDIDATE_SCAN = 40;
const MAX_CLICKS_PER_MENU = 36;
...
function addIssue(p) {
  ...
  seenIssue.add(key);
  const id = `ISSUE-SA-SET-${String(issues.length + 1).padStart(2, '0')}`;
...
async function exploreMenu(page, state, menu) {
  ...
  if (menu.label === 'Settings') {
    for (const sub of SETTINGS_SUB) {
      ...
      // Nested deep on every sub — SHORT budgets already applied
...
function issueMd(iss) {
  return [
    ...
    `- **Screenshot:** \`${iss.screenshot ? `docs/e2e-ui-screenshots/stress/sa-settings-admin/${iss.screenshot}` : 'n/a'}\``,
...
function writeFindings() {
  ...
  const lines = [];
  lines.push(`# ${SECTION} — SUPER_ADMIN Settings/Admin Stress+Deep E2E`);
  lines.push('');
  lines.push(`> Generated: ${new Date().toISOString()}`);
  lines.push(`> Role: \`${ROLE}\` (\`${EMAIL}\` / tenant \`${TENANT}\`)`);
  lines.push(`> UI: \`${FE}\` · API: \`${API}\``);
  lines.push('> Tool: Playwright Chromium · SHORT deep + rapid settings stress');
  lines.push(`> Screenshots: \`docs/e2e-ui-screenshots/stress/sa-settings-admin/\``);
  ...
  lines.push(`| Stress rapid navs | **${counters.stressNavs}** |`);
  ...
  lines.push('');
  lines.push('## Stress: rapid 5 settings routes');
  lines.push('');
  lines.push(`Routes: ${STRESS_SETTINGS.map((s) => `\`${s}\``).join(' → ')}`);
  lines.push('');
  if (!stressEvents.length) lines.push('_No stress events._');
  for (const e of stressEvents) {
    lines.push(
      `- **${e.sub}**: ${e.ok ? 'OK' : 'FAIL'} ${e.ms}ms · netFails=${e.newFails ?? 0} · console=${e.newConsole ?? 0}${e.err ? ` · ${e.err}` : ''}`,
    );
    if (e.fails?.length) {
      for (const f of e.fails) lines.push(`  - net: \`${f}\``);
    }
    if (e.console?.length) {
      for (const c of e.console) lines.push(`  - console: ${c}`);
    }
  }
  lines.push('');
  lines.push('## Issues');
...
function appendContract(file, heading, bodyLines) {
  ...
  if (existing.includes(marker)) {
    ...
    existing = existing.replace(re, `${bodyLines.join('\n').trim()}\n\n`);
  } else {
    if (!existing.trim()) {
      existing =
        file.includes('BACKEND')
          ? '# E2E_STRESS_BACKEND_CONTRACT\n\n> Stress + deep E2E shards against local FE:3001 → BE:4000 → Hostinger tunnel\n> **No Render. No git commit.**\n\n'
          : '# E2E_STRESS_FRONTEND_CONTRACT\n\n> Stress + deep E2E shards against local FE:3001 → BE:4000 → Hostinger tunnel\n> **No Render. No git commit.**\n\n';
    }
...
function writeContracts(be, fe) {
  const commonHead = [
    `## ${SECTION}`,
    '',
    `> Tester: \`${EMAIL}\` (${ROLE}) · tenant \`${TENANT}\` · ${new Date().toISOString().slice(0, 10)}`,
    `> Evidence: \`docs/e2e-ui-screenshots/stress/sa-settings-admin/\` (**${counters.screenshots}** PNGs + FINDINGS.md)`,
    `> Depth: menus=${counters.menus} settingsSubs=${counters.settingsSubs} clicks=${counters.controlsClicked} maxDepth=${maxDepthReached} stressNavs=${counters.stressNavs}`,
    `> Stress: rapid open ${STRESS_SETTINGS.join(' → ')}`,
    '',
  ];

  const beLines = [...commonHead];
  const beIssues = be.length ? be : [];
  if (!beIssues.length) {
    beLines.push('_No backend issues unique to this shard (or none captured)._');
    beLines.push('');
  } else {
    for (const iss of beIssues) beLines.push(issueMd(iss));
  }
  // Always record stress network summary for BE contract
  beLines.push('### Stress network summary');
  beLines.push('');
  const allStressFails = stressEvents.flatMap((e) => e.fails || []);
  if (!allStressFails.length) beLines.push('_No HTTP ≥400 during rapid-5 settings nav._');
  else for (const f of allStressFails.slice(0, 20)) beLines.push(`- \`${f}\``);
  beLines.push('');

  const feLines = [...commonHead];
  const feIssues = fe.length ? fe : [];
  if (!feIssues.length) {
    feLines.push('_No frontend issues unique to this shard (or none captured)._');
    feLines.push('');
  } else {
    for (const iss of feIssues) feLines.push(issueMd(iss));
  }
  feLines.push('### Stress console summary');
  feLines.push('');
  const allCons = stressEvents.flatMap((e) => e.console || []);
  if (!allCons.length) feLines.push('_No console errors during rapid-5 settings nav._');
  else for (const c of allCons.slice(0, 20)) feLines.push(`- ${c}`);
  feLines.push('');

  appendContract('E2E_STRESS_BACKEND_CONTRACT.md', SECTION, beLines);
  appendContract('E2E_STRESS_FRONTEND_CONTRACT.md', SECTION, feLines);
}
...
async function main() {
  ...
  try {
    ...
    // Permissions first
    await exploreMenu(page, state, MENUS[0]);
    // Settings + all subs
    await exploreMenu(page, state, MENUS[1]);
    // Stress rapid 5
    await stressRapidSettings(page, state);
    // Remaining admin menus
    for (const m of MENUS.slice(2)) {
  ...
  const { be, fe } = writeFindings();
  writeContracts(be, fe);
  logLine(
