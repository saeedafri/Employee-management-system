...
- **Staged but uncommitted**: `AGENTS.md` (new file)
...
For every new backend/frontend task, Codex must follow this workflow before editing source code:
...
Codex must not say PASS unless the final-report-enforcer confirms:
...
4. Documentation updated (API_MAPPING, Swagger, AGENTS.md).
...
**Safety hooks active** — `.Codex/hooks/ems-safety-guard.cjs` blocks these commands automatically:
